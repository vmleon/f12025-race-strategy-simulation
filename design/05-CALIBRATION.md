...
| 5    | Pit stop duration (per regime)              | 3           | Mean time loss vs baseline, races only; deterministic (no variance fitted)                               |

All steps filter by `ai_controlled` to produce separate PLAYER and AI coefficients (see Dual Calibration above).

### Pit Stop Grouping

Pit stop duration is fitted from **race sessions only** (`session_type IN (10, 11, 12, 15, 16, 17)`) — in Free Practice and Qualifying a "pit stop" is a garage return (the car parks for an arbitrary time between runs), which is not a real pit-lane time loss. Within a race, each stop's loss is reconstructed from the pit-entry sector (`pit_status = 1`) **plus the following out-lap sector(s) while they stay clearly inflated** (>30% over the normal-sector baseline, capped at 4 sectors). This walk-forward is necessary because `pit_status = 2` ("in pit area") is **never recorded**: snapshots are written on sector transitions, so the stationary box stop and pit-exit time land in an out-lap sector that the game has already flagged back to `pit_status = 0`. Summing only the `pit_status = 1` entry sector would capture ~3 s instead of the real ~20 s. The loss is the sum of (`sector_time − baseline`) across those sectors. Only the **mean** is persisted (`pit_stop_time_loss`) — pit-lane loss is near-deterministic per track (fixed lane length + speed limit), so no variance/stddev is fitted and the engine applies the mean directly. Because the player rarely accumulates enough race stops to clear the sample threshold, the simulator **borrows the AI fit** for the player whenever the player's value is still the cold-start prior.
